...
用户会在多个 Agent 之间循环切换。将 `交班` 和 `接班` 视为固定命令，交班文档属于私有运行上下文，不得提交 Git。

#### 用户说“交班”时

1. 以最近一次完成的“接班”为起点；如果本轮没有 handoff 文档，则以用户明确要求当前 Agent 接替上一 Agent 的消息为起点。
2. 只总结该起点至当前消息之间新增的上下文，不重复更早历史。
3. 将 Markdown 写入仓库根目录 `.tmp/.tmp_chats/`，文件名使用 `handoff-YYYYMMDD-HHMMSS.md`；该目录由 `.gitignore` 排除。
4. 文档至少包含：覆盖时间范围、用户长期偏好与约定、新增或修改的常规任务、已执行的重要操作、当前服务状态、未完成事项、工作树状态、后续建议和相关文件入口。
5. 对文件传输等一次性任务只保留可复用规则和必要结果，不堆叠终端流水账。
6. 不写入 token、密码、代理认证信息、open ID、SSH 私钥或其他秘密；内部地址和路径仅在继续任务确有必要时记录。
7. 写完后向用户返回 handoff 文档的绝对路径，不发送消息给另一 Agent，也不擅自提交该文档。

#### 用户说“接班”并提供 handoff 文档时

1. 完整读取文档，并把其中的终点作为当前任期起点。
2. 对照仓库工作树、关键进程、服务状态及明确列出的未完成事项做只读核验；不得假定文档中的瞬时状态仍然成立。
3. 继承长期约定，但不要重做已经完成的一次性操作。
4. 保护文档中指出的未提交改动；未经核验不得覆盖、丢弃或混入无关提交。
5. 简要向用户确认接班范围、当前可继续的能力和发现的状态偏差，然后直接承接后续任务。
