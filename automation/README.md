...
For recurring Git alias maintenance, configure `git_alias_sync` privately and run `python3 automation/scripts/sync_git_aliases.py --commit-message "feat: update git aliases"`. The command validates the Bash script, optionally commits and pushes its source repository, copies it to every configured target with size and SHA-256 verification, installs it locally and remotely, and confirms that configured aliases have identical values. Source repositories, machine names, and target paths remain private configuration.
