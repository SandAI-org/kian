...
For a recurring wheel mirror, configure `wheel_sync.profiles` privately and run `python3 automation/scripts/sync_wheels.py <profile>`. The command discovers the newest wheel in every configured dist directory before transferring anything and stages each wheel locally once. After all downloads verify successfully, it removes older wheels for the same distributions from each target directory, then uploads and verifies the selected wheels on every destination. This keeps wildcard installs from seeing multiple versions of one package. Use a separate profile and target directory for each base image.
