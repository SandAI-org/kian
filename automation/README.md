...
For a recurring wheel mirror, configure `wheel_sync.profiles` privately with an explicit `stable_versions` mapping and run `python3 automation/scripts/sync_wheels.py <profile>`. The command selects only the pinned version for every distribution; newer or more recently modified test builds are never promoted automatically. A missing or ambiguous pinned wheel fails before any deletion or transfer. After all stable wheels verify locally, the command removes other versions of the same distributions from each target, then uploads and verifies the pinned wheels. Change a pin only after the user explicitly confirms that package's new stable version. Use a separate profile and target directory for each base image.
