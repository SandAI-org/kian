...
常规 wheel 镜像同步需在私有配置中定义 `wheel_sync.profiles`，然后运行 `python3 automation/scripts/sync_wheels.py <profile>`。脚本会先在每个配置的 dist 目录发现最新 wheel，确认完整后才开始传输；每个 wheel 只在本机暂存一次，随后上传并校验所有目标。每种基础镜像应使用独立 profile 和目标目录。
